...
static void sweep2wake_vert_func(int x, int y)
{
	...
	//printk("[sweep2wake_vert_func] firsty:%d\n", firsty);
			...
			if (barriery[0] == true || (y > prevy && y < nexty)) {
				...
				if (barriery[1] == true || (y > prevy && y < nexty)) {
					...
					if (y > prevy) {
						if (y > (nexty + 150)) {
							if (exec_county) {
								pr_info("wake_gesture: sweep down\n");
								wake_lock_timeout(&l2w_wakelock, HZ/2);
								report_gesture(4);
								exec_county = false;
							}
...
static void sweep2wake_horiz_func(int x, int y)
{
	...
	//printk("[sweep2wake_horiz_func] firstx:%d\n", firstx);
		...
		if (barrierx[0] == true || (x > prevx && x < nextx)) {
			...
			if (barrierx[1] == true || (x > prevx && x < nextx)) {
				...
				if (x > prevx) {
					if (x > (nextx + 150)) {
						if (exec_countx) {
							pr_info("wake_gesture: sweep right\n");
							wake_lock_timeout(&l2w_wakelock, HZ/2);
							report_gesture(1);
							exec_countx = false;
						}
	...
	//right->left
	} else if (firstx >= 900 && y < 1820) {
		prevx = firstx;
		nextx = prevx - 150;
		if ((barrierx[0] == true) ||(x < prevx && x > nextx && y < 1820)) {
			prevx = nextx;
			nextx -= 150;
			barrierx[0] = true;
			if ((barrierx[1] == true) || (x < prevx && x > nextx  && y < 1820)) {
				prevx = nextx;
				barrierx[1] = true;
				if (x < prevx  && y < 1820) {
					if (x < (nextx - 150)) {
						if (exec_countx) {
							pr_info("wake_gesture: sweep left\n");
							wake_lock_timeout(&l2w_wakelock, HZ/2);
							report_gesture(2);
							exec_countx = false;
						}
...
{
        if (!gestures_switch && dt2w_switch == 1 && (y > 0 && y < 1820)) {
                reset_dt2w();
                return;
        }
...
static int synaptics_ts_suspend(struct i2c_client *client, pm_message_t mesg)
{
...
#ifdef CONFIG_TOUCHSCREEN_SYNAPTICS_SWEEP2WAKE
	if (s2w_switch == 1 || dt2w_switch > 0 || gestures_switch == 1) {
	...
	if (ts->use_irq) {
#ifdef CONFIG_TOUCHSCREEN_SYNAPTICS_SWEEP2WAKE
		if ((s2w_switch == 2 || s2w_switch == 0) && dt2w_switch == 0 && gestures_switch == 0) {
#endif
...
static int synaptics_ts_resume(struct i2c_client *client)
{
	...
	if (s2w_switch == 1 || dt2w_switch > 0 || gestures_switch == 1)
        ...
        if ((s2w_switch == 2 || s2w_switch == 0) && dt2w_switch == 0 && gestures_switch) {
#endif 
	if (ts->power) {
		ts->power(1);
		hr_msleep(100);
