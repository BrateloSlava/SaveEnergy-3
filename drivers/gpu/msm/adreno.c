		   ...
		   struct kgsl_context *context)
{
	...
	if (context) {
		...
		KGSL_FT_CRIT(device, "Last context unknown\n");
	}
			...
			struct adreno_ft_data *ft_data)
{
	...
	if (adreno_dev == NULL) {
		KGSL_FT_CRIT(device, "_adreno_ft: adreno_device unknown \n");
		return -EINVAL;
	}
	rb = &adreno_dev->ringbuffer;
	last_active_ctx = adreno_dev->drawctxt_active;
	context = idr_find(&device->context_idr, ft_data->context_id);
	if (context == NULL) {
		...
		KGSL_FT_ERR(device, "Last no adreno context, kgsl context id:%d\n",
			context->id);
		goto play_good_cmds;
	} else {
		adreno_context = context->devctxt;
